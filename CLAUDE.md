...
- **Launch cell (Celda 8)**: reads `LTX_MODEL_CHOICE` (falls back to its `GGUF_QUANT` param) and exports `DISABLE_ENHANCER` (checkbox, default on — the sulphur enhancer model + llama.cpp don't fit free Colab's disk). GGUF-specific patches go through `gpatch()` and are skipped entirely for `FP8_ORIGINAL` (original checkpoint, no Kijai VAEs); the custom-LoRA installer hook (P9) and the Gradio `share=True` hook always apply.