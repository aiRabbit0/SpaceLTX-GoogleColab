...
1. **The Colab notebook patches `app.py` by exact string replacement.** The launch cell ("Celda 8") reads the downloaded `app.py` (= this repo's `app_space.py`) and calls `patch(old, new, label)` / `gpatch(...)` (GGUF-only variant, skipped when `FP8_ORIGINAL` is selected) with verbatim source snippets: the fp8-checkpoint entry in `DOWNLOADS`, loader node handling, the `*_LORA_FILENAME` constants and per-slot `DOWNLOADS` blocks, **and the Gradio slider `label="..."` lines for the 7 swappable LoRA slots** (`slot1`–`slot7`; video + audio, renamed to `custom: <name>` from CivitAI metadata; `slot7` has no original lora and no `DOWNLOADS` block). Any edit to `app_space.py` that touches a patch-target string must update the corresponding `old` string in the notebook, or the patch silently degrades (prints `⚠️ parche ... NO aplicado`).
...
Injectors, each toggled by UI inputs: `_inject_params` (prompt, image, seed, dimensions, mode, likeness/anchor strengths), `_inject_optional_loras` (14 video + 14 audio strength sliders; all but `slot7` driven by `PRESET_VALUES` presets — `slot7` is deliberately outside the preset wiring so switching presets never resets it), `_inject_refine_sigmas`, `_inject_msr` (multi-subject reference), `_inject_prompt_relay`, `_inject_scene_chain`, `_inject_kv_conditioning`, `_inject_audio_reference`.
...
Cells in order: GPU/CUDA check → tokens (HF + CivitAI, enables `hf_transfer`) → download code from this repo's GitHub `main` (`app_space.py` → `app.py`, `workflow_runexx.json` → `runexx_msr_workflow.json`) + install deps (torch 2.8.0 with cu-tag fallback) → mock the `spaces` module (no-op `@spaces.GPU`) → optional CivitAI LoRA replacement of 7 slots (`slot1`–`slot7`; legacy slot names accepted via `_LEGACY_SLOTS`) → VRAM/disk monitor → **environment-validated model selector** → patches + launch → disk cleanup → GPU release.
