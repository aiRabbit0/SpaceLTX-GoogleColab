...
Cells in order: GPU/CUDA check → tokens (HF + CivitAI, enables `hf_transfer`) → download code from this repo's GitHub `main` (`app_space.py` → `app.py`, `workflow_runexx.json` → `runexx_msr_workflow.json`) + install deps (torch 2.8.0 with cu-tag fallback) → mock the `spaces` module (no-op `@spaces.GPU`) → optional CivitAI LoRA downloads (additive catalog; selection happens in the app UI) → VRAM/disk monitor → **environment-validated model selector** → patches + launch → disk cleanup → GPU release.

- **LoRA cell (Celda 5)**: takes a plain list of CivitAI URLs/version ids, resolves each version, fetches metadata via `/api/v1/model-versions/{id}` (name, base model, trained words, size; warns if base model isn't LTX), downloads to `/content/custom_loras/` (atomic `.part` rename + Content-Length check + text/html guard for gated models), and accumulates `mapping.json` keyed by filename. The launch cell's installer hook links the files into `models/loras/ltx23/custom/` and copies the manifest as `manifest.json` so the app's slot dropdowns show CivitAI names. Originals are always downloaded; nothing is replaced.
...
- **Launch cell (Celda 8)**: reads `LTX_MODEL_CHOICE` (falls back to its `GGUF_QUANT` param) and exports `DISABLE_ENHANCER` (checkbox, default on — the sulphur enhancer model + llama.cpp don't fit free Colab's disk) plus `SKIP_SLOT_LORAS` (per-slot checkboxes `SLOT1_HARDCUT`…`SLOT6_PHYSICS_V2`, default all on; unchecked slots skip the original lora download in `_ensure_models` — entries carry a `"slot"` key — and their UI dropdown starts at `(none)`). It also self-mocks the `spaces` module if missing (kernel restart resilience). GGUF-specific patches go through `gpatch()` and are skipped entirely for `FP8_ORIGINAL` (original checkpoint, no Kijai VAEs); the custom-LoRA installer hook (P9) and the Gradio `share=True` hook always apply.